...
from server.core.handlers.multi import MultiHandler
#
...
class MultiShell(cmd.Cmd):
    ...
    def list_sessions(self, verbose=False):
        ...
        for orch_id, shell in self.shells.items():
            # print shell.sysinfo
            try:
    ...
    def do_session(self, line):
        ...
        try:
            ...
            shell = list(self.shells.values())[i]
            return shell.start(False)
        except Exception as e:
            # print e
            pass
        if args.session_id:
            args.session_id = 'b\'' + args.session_id if not args.session_id.startswith('b\'') else args.session_id
    ...
    def do_handler(self, line):
        ...
        parser_add.add_argument("SCRIPT", help="The file that contains the Handler in Python 'server.core' code", type=str)
        parser_add.add_argument("ARGUMENTS", help="The arguments passed to the Python 'server.core' handler script", type=str, default='', nargs='*')
        parser_add.add_argument("--shell", '-s', help="The argument in the Python code that contains the 'server.core.shell.baseshell.BaseShell' implementation",
