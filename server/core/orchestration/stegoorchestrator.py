...
from server.core.datamanipulation import AdHocChunker
from server.core.datamanipulation import Chunker

from server.core.orchestration import StreamIdentifier
from server.core.orchestration import Orchestrator

from server.core.datamanipulation import StegoInjector, DataTransformer

from string import ascii_letters

from os import urandom

from server.core.helpers import copydoc
...
class StegoOrchestrator ( Orchestrator ) :
	"""
The `StegoOrchestrator` class combines compression, chunking, encryption, stream tagging and steganography injection, by utilizing the below ``server.core`` classes:

 - :class:`server.core.datamanipulation.AdHocChunker`
 - :class:`server.core.datamanipulation.Compressor`
 - :class:`server.core.crypto.keys.StandardCyclingKey`
 - :class:`server.core.orchestration.StreamIdentifier`
 - :class:`server.core.datamanipulation.StegoInjector`
 - :class:`server.core.datamanipulation.DataTransformer`
...
The `StegoOrchestrator` packs `(stream, message)` pairs in predefined data templates.
	...
	def __init__( self, passphrase, stego_config, main_template, transformation_list = [], tag_length = 2, cycling_algorithm = None, streams = [], hex_inject = False, reverse = False ) :
		"""
:param str stego_config: The configuration that is passed to :class:`server.core.datamanipulation.stegoinjector.StegoInjector`.
:param str main_template: The default template that will be used in :func:`readyMessage()` `template` argument.
:param list transformation_list: The Transformation List that is passed to the :class:`server.core.datamanipulation.datatransformer.DataTransformer` object.
:param class cycling_algorithm: The hashing/cycling function used in all OTP crypto and stream identification. If not specified the  :class:`server.core.crypto.algorithms.StandardCyclingAlgorithm` will be used. The :class:`hashlib.sha256` is a great choice if `hashlib` is available.
