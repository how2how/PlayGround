...
class StageableHandler ( FunctionDictHandler ) :
	"""
The StageableHandler is a :class:`server.core.handlers.FunctionDictHandler` that can load payloads (stages) during execution. Additional functions can be sent in a serialized form (ready stages can be found in :mod:`server.core.payloads`).
The stage function have to be implemented according to :class:`server.core.handlers.FunctionDictHandler` documentation.

To running `StageableHandler`s, additional functions can be packed with the :func:server.core.handlers.`StageableHandler.createStageMessage` and sent like normal messages with a `sendAdHoc` call.
"""
